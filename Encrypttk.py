...
def aesEncrypt(PassPhrase, message):
    ...
    while(len(PassPhrase)!=16):
        ...
        #PassPhrase=input()#input your message 
    ...
    #I put the print here so that my text would have no loop
    ...
    for y in range(1, loopmsg): # loop to encrypt all segments of the message
        ...
        #print your final result(Cipher Text)
        #print(outputhextemp)
        displayLabel.insert(END, outputhextemp ) #displays the result into the textbox(displayLabel)
        
        start = start + 16 #increment start pointer
        end = end + 16 #increment end pointer

#tkinter program starts from here
root = Tk()
root.title("AES ENCRYPTION AND DECRYPTION GUI")
root.geometry("600x400")

label = Label(root, text="AES ENCRYPTION" , font="20")
label.grid(row=0, column=1,sticky=W)

label4 = Label(root, text="" , font="20")
label4.grid(row=0, column=3)

inputText2 = Label(root, text= "")
inputText2.grid(row=1, column=1, padx=0, pady=0)

inputText1 = Label(root, text= "Enter PlainText Message: ") # input plaintext message
inputText1.grid(row=2, column=0, sticky=NW)

text = Text(root, height=10, width= 60)
text.grid(row=2, column=1)

inputText2 = Label(root, text= "")
inputText2.grid(row=3, column=0, padx=0, pady=0)


inputText3 = Label(root, text= "Enter PassKey(key should be in 16 bits or less): ")#input passkey
inputText3.grid(row=4, column=0, padx=0, pady=0)
Passphrase = StringVar()
entry = Entry(root, textvariable=Passphrase)
...
def myClick():
    ...
    #result = aesEncrypt(PassPhrase,message)
    aesEncrypt(PassPhrase,message)
    #displayLabel.config(text=result)
    #displayLabel2.config(text= text.get("1.0", "end"))


button = Button(root, text="Click", command=myClick)
button.grid(row=5, column=1, sticky=S)

filler = Label(root, fg="blue").grid(row=6,column=1)
# displayLabel = Label(root, fg="blue")
# displayLabel.grid(row=7,column=1)
displayLabel = Text(root, fg="green", height=15, width= 50)
displayLabel.grid(row=7,column=1)

# displayLabel2 = Label(root, fg="blue")
# displayLabel2.grid(row=7,column=1)
resultText1 = Label(root, text= "Your CipherText Message : ")
resultText1.grid(row=7, column=0, padx=0, pady=0, sticky=W)
